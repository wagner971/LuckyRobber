...
func capture() -> void:
	...
	game.store = SaveStore.new("res://tests/vikings_visual.json")
	...
	game.store.data.upgrades = {"strength":5,"grip":12,"carry":12,"capacity":20,"noise":10}
	root.add_child(game)
	game.ui.jobs_index = Balance.LOCATION_ORDER.find("vikings")
	game.action("locations")
	await shot("res://tests/vikings_jobs.png")
	game.start_run("vikings")
	game.run.set_physics_process(false)
	game.level.player.set_physics_process(false)
	await shot("res://tests/vikings_gameplay.png")
	game.level.player.position = Vector3(1.2,0,-6.1)
	await shot("res://tests/vikings_bow.png")
	game.action("locations")
	root.size = Vector2i(360,800)
	await shot("res://tests/vikings_jobs_tall.png")
	game.free()
	print("VIKING UI CAPTURED")
	quit()
