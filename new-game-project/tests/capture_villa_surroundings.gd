...
func capture() -> void:
	...
	game.store = SaveStore.new("res://tests/villa_surroundings_visual_profile.json")
	...
	await save_frame("res://tests/villa_follow_center_450x800.png")
	for edge in [
		["left", Vector3(-5.5, 0, -4.0)],
		["right", Vector3(5.5, 0, -4.0)],
		["rear", Vector3(0, 0, -8.0)],
		["van", Vector3(0, 0, 8.0)],
	]:
		game.level.player.position = edge[1]
		game.camera_juice._process(3.0)
		await save_frame("res://tests/villa_follow_%s_450x800.png" % edge[0])
	game.free()
	print("VILLA SURROUNDINGS CAPTURES COMPLETE")
	quit()
