...
func capture() -> void:
	...
	game.store = SaveStore.new("res://tests/laboratory_visual_profile.json")
	...
	await save_frame("res://tests/laboratory_follow_center_450x800.png")
	for edge in [
		["left", Vector3(-5.7, 0, -3.2)],
		["right", Vector3(5.7, 0, -3.2)],
		["rear", Vector3(0, 0, -10.0)],
		["van", Vector3(0, 0, 8.4)],
	]:
		game.level.player.position = edge[1]
		game.camera_juice._process(3.0)
		await save_frame("res://tests/laboratory_follow_%s_450x800.png" % edge[0])
	game.free()
	print("LABORATORY FOLLOW CAPTURES COMPLETE")
	quit()
