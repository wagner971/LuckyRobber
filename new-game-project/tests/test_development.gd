...
func test() -> void:
	...
	check(find_button(game.ui.menu, "DEV · MAX ALL + UNLOCK AVAILABLE LEVELS") == null, "Home keeps DEV out of the normal composition")
	game.action("settings")
	find_button(game.ui.menu, "DEV TOOLS ▾").pressed.emit()
	var button = find_button(game.ui.menu, "DEV · MAX ALL + UNLOCK AVAILABLE LEVELS")
	...
	check(game.store.data.unlocked == Balance.LOCATION_ORDER.slice(0, Balance.LOCATION_ORDER.find("pyramid")), "DEV unlocks Chapter 1 but keeps Pyramid and Castle behind Museum")
	game.start_run("pyramid")
	check(game.screen == "settings" and game.run == null, "Direct Pyramid launch stays blocked before Museum completion")
	check(game.store.data.wallet == 0 and Progression.objective_count(game.store.data, "museum") == 0 and not Progression.campaign_cleared(game.store.data), "DEV grants no money, objectives or campaign")
	check(find_button(game.ui.menu, "DEV · MAXED + AVAILABLE LEVELS UNLOCKED").disabled, "Button confirms MAX and disables repeated use")
	...
	game.run.abandon()
	game.action("shop")
	check(find_button(game.ui.menu, "DEV · MAX ALL + UNLOCK AVAILABLE LEVELS") == null, "DEV remains hidden in the shop")
	...
	root.add_child(game)
	check(find_button(game.ui.menu, "DEV · MAX ALL + UNLOCK AVAILABLE LEVELS") == null, "DEV hidden when development mode disabled")
	game.action("dev_max")
