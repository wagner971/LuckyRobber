...
func capture() -> void:
	...
	root.add_child(game)
	for page in ["home","shop","locations"]:
		game.open_menu(page)
		await create_timer(0.9).timeout
		await RenderingServer.frame_post_draw
		root.get_texture().get_image().save_png("res://tests/vivid_%s_%s.png" % [page,suffix])
	for location in ["apartment","electronics","pyramid","castle"]:
		...
		await RenderingServer.frame_post_draw
		root.get_texture().get_image().save_png("res://tests/vivid_%s_%s.png" % [location,suffix])
		game.cleanup_run()
	game.free()
	quit()
