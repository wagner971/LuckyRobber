...
func capture() -> void:
	...
	game.store = SaveStore.new("res://tests/laboratory_jobs_polish_profile.json")
	game.store.session_only = true
	game.store.data.tutorial_completed = true
	game.store.data.noise_tutorial_completed = true
	game.store.data.first_job_at = 1
	game.store.data.unlocked = ["apartment","house","villa","electronics","mansion","laboratory"]
	game.store.data.apartment_final_job_completed = true
	root.add_child(game)
	game.ui.jobs_index = 5
	game.action("locations")
	await process_frame
	await process_frame
	await RenderingServer.frame_post_draw
	root.get_texture().get_image().save_png("res://tests/laboratory_jobs_polish_450x800.png")
	game.free()
