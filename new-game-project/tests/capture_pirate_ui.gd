...
func capture() -> void:
	...
	game.store = SaveStore.new("res://tests/pirate_visual.json")
	...
	game.store.data.upgrades = {"strength":5,"grip":11,"carry":11,"capacity":20,"noise":10}
	root.add_child(game)
	game.ui.jobs_index = Balance.LOCATION_ORDER.find("pirate_ship")
	game.action("locations")
	await shot("res://tests/pirate_jobs.png")
	game.start_run("pirate_ship")
	game.run.set_physics_process(false)
	game.level.player.set_physics_process(false)
	await shot("res://tests/pirate_gameplay.png")
	game.level.player.position = Vector3(1.2,0,-6.1)
	await shot("res://tests/pirate_bow.png")
	game.action("locations")
	root.size = Vector2i(360,800)
	await shot("res://tests/pirate_jobs_tall.png")
	game.free()
	print("PIRATE UI CAPTURED")
	quit()
